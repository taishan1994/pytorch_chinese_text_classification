...
class Trainer:
    def __init__(self, args, train_loader, dev_loader, test_loader):
        ...
        if args.use_pretrained:
            ...
            self.model = textcnn.TextCnn(args, pretrained_embedding)
        else:
 ...
 micro_f1, macro_f1))
                    if micro_f1 > best_dev_micro_f1:
                        ...
                        checkpoint_path = os.path.join(self.args.output_dir, 'testcnn_best.pt')
    ...
    def predict(self, tokenizer, text, word2id, id2label, args):
        model = self.model
        optimizer = self.optimizer
        checkpoint = os.path.join(args.output_dir, 'textcnn_best.pt')
        ...
        with torch.no_grad():
            ...
            if len(word2id) < args.max_seq_len:
...
if __name__ == '__main__':
    ...
    utils.set_logger(os.path.join(args.log_dir, 'textcnn_main.log'))
    ...
    # 测试
    # logger.info('========进行测试========')
    # checkpoint_path = '../checkpoints/textcnn_best.pt'

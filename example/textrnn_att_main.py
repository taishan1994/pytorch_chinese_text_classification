...
class Trainer:
    def __init__(self, args, train_loader, dev_loader, test_loader):
        ...
        if args.use_pretrained:
            ...
            self.model = textrnn_att.TextRnnAtt(args, pretrained_embedding)
        else:
            self.model = textrnn_att.TextRnnAtt(args)
 ...
 micro_f1, macro_f1))
                    if micro_f1 > best_dev_micro_f1:
                        ...
                        checkpoint_path = os.path.join(self.args.output_dir, 'textrnn_att_best.pt')
    ...
    def predict(self, tokenizer, text, word2id, id2label, args):
        model = self.model
        optimizer = self.optimizer
        checkpoint = os.path.join(args.output_dir, 'textrnn_att_best.pt')
...
if __name__ == '__main__':
    args = textrnn_att_config.Args().get_parser()
    utils.set_seed(args.seed)
    utils.set_logger(os.path.join(args.log_dir, 'textrnn_att_main.log'))
    ...
    # 测试
    logger.info('========进行测试========')
    checkpoint_path = '../checkpoints/textrnn_att_best.pt'

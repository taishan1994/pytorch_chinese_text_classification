...
from configs import textrcnn_config
from preprocess import processor_word
from dataset import dataset_word
...
class Trainer:
    def __init__(self, args, train_loader, dev_loader, test_loader):
        ...
        if args.use_pretrained:
            ...
            self.model = textrcnn.TextRcnn(args, pretrained_embedding)
        else:
            self.model = textrcnn.TextRcnn(args)
    ...
    def train(self):
        ...
        for epoch in range(args.train_epochs):
            for train_step, train_data in enumerate(self.train_loader):
                ...
                train_outputs = self.model(word_ids)
 ...
 micro_f1, macro_f1))
                    if micro_f1 > best_dev_micro_f1:
                        ...
                        checkpoint_path = os.path.join(self.args.output_dir, 'textrcnn_best.pt')
    ...
    def predict(self, tokenizer, text, word2id, id2label, args):
        model = self.model
        optimizer = self.optimizer
        checkpoint = os.path.join(args.output_dir, 'textrcnn_best.pt')
        ...
        with torch.no_grad():
            ...
            outputs = model(word_ids)
            outputs = np.argmax(outputs.cpu().detach().numpy(), axis=0)
            outputs = id2label[outputs]
            return outputs
...
if __name__ == '__main__':
    args = textrcnn_config.Args().get_parser()
    utils.set_seed(args.seed)
    utils.set_logger(os.path.join(args.log_dir, 'textrcnn_main.log'))

    processor = processor_word.Processor()
    ...
    train_out = processor_word.get_out(processor, '../data/cnews/raw_data/train.txt', args, word2id, 'train')
    train_features, train_callback_info = train_out
    train_dataset = dataset_word.ClassificationDataset(train_features)
    ...
    dev_out = dataset_word.get_out(processor, '../data/cnews/raw_data/dev.txt', args, word2id, 'dev')
    dev_features, dev_callback_info = dev_out
    dev_dataset = dataset_word.ClassificationDataset(dev_features)
    dev_loader = DataLoader(dataset=dev_dataset,
                            batch_size=args.eval_batch_size,
                            num_workers=2)

    test_out = dataset_word.get_out(processor, '../data/cnews/raw_data/test.txt', args, word2id, 'test')
    test_features, test_callback_info = test_out
    test_dataset = dataset_word.ClassificationDataset(test_features)
    ...
    # 测试
    logger.info('========进行测试========')
    checkpoint_path = '../checkpoints/textrcnn_best.pt'
